...
public:
     ...
     *
     * @param _thread_count The number of threads to use. The default value is the total number
     *  of hardware threads available, as reported by the implementation.
     *  With a hyperthreaded CPU, this will be twice the number of CPU cores.
     ...
     * Note that if the variable paused is set to true, then any tasks still in the queue will never be executed.
     */
    ~thread_pool()
    {
        wait_for_tasks();
     ...
     * However, if the variable paused is set to true, this function only waits for the currently running tasks
     * (otherwise it would wait forever). To wait for a specific task, use submit() instead, and call the wait()
     ...
     * execute std::this_thread::yield() if there are no tasks in the queue. The default value is 1000.
     */
...
private:
    ...
    /**
     * @brief An atomic variable indicating to the workers to keep running. When set to false,
     *  the workers permanently stop working.
     */
